...
NVIM="https://pub.mtdcy.top:8443/cmdlets/latest/$ARCH/app/nvim/nvim-0.10.3-1.tar.gz"

locally=0
if curl --fail -sIL https://git.mtdcy.top; then
    locally=1
fi
...
if [ "$0" = "install" ] || [ "$0" = "bash" ]; then
    if [ -d "$HOME/.nvim" ]; then
        ...
        if [ "$locally" -eq 1 ]; then
            git clone https://git.mtdcy.top:8443/mtdcy/pretty.nvim.git "$HOME/.nvim"
        else
...
if curl --fail -sIL -o /dev/null "$NVIM"; then
    mkdir -pv "prebuilts/$ARCH"
    info "== fetch $NVIM"
    curl -sL -# "$NVIM" | tar -x -C "prebuilts/$ARCH"
else
